 ...
 * Base Strategy
 *
 * Abstract base class for agent strategies with shared helper methods.
 * Concrete strategies (Academic, Degen) extend this class.
 */
...
export interface Strategy {
  ...
  /**
   * Main decision function - evaluates state and returns actions
   */
  decide(state: AgentState, config: AgentConfig): DecisionResult;
}
...
export abstract class BaseStrategy implements Strategy {
  ...
  /**
   * Confidence multiplier for this strategy (override in subclasses)
   * Academic strategies should have higher base confidence requirements
   * Degen strategies can work with lower confidence
   */
  protected abstract readonly confidenceMultiplier: number;
  ...
  decide(state: AgentState, config: AgentConfig): DecisionResult {
    ...
    if (this.checkLoanHealth(state, config, thinking, actions)) {
      urgent = true;
    }

    // Priority 9: Treasury low
    if (this.checkTreasuryLow(state, config, thinking, actions)) {
      urgent = true;
    }

    // Priority 6: Repay loan if excess funds
    this.checkRepayLoan(state, config, thinking, actions);

    // Priority 5: Claim revenue
    this.checkClaimRevenue(state, config, thinking, actions);

    // Priority 4: Buy opportunities
    this.evaluateBuyOpportunities(state, config, thinking, actions);

    // Priority 3: Sell underperformers
    this.evaluateSellOpportunities(state, config, thinking, actions);
  ...
  protected calculateRepayToSafe(loan: LoanInfo, balance: bigint): bigint {
    // Repay 30% of borrowed to get back to safety
    const repayAmount = (loan.borrowedAmount * BigInt(30)) / BigInt(100);
    return balance >= repayAmount ? repayAmount : balance;
  }

  // ============================================
  // Decision Logic (can be overridden)
  // ============================================

  protected checkLoanHealth(
    state: AgentState,
    config: AgentConfig,
    ...
    thinking.push({
      category: "health",
      observation: `Active loan: ${this.formatUsdc(borrowedAmount)} borrowed, health factor: ${healthFactor.toFixed(2)}, LTV: ${currentLTV}%`,
      conclusion:
    ...
    if (healthFactor < HEALTH_FACTOR_CRITICAL) {
      const repayAmount = this.calculateRepayToSafe(state.loan, state.usdcBalance);
      ...
      if (repayAmount > BigInt(0)) {
        actions.push({
          ...
          reason: `Critical health factor (${healthFactor.toFixed(2)}), repaying ${this.formatUsdc(repayAmount)}`,
          confidence: 0.95 * this.confidenceMultiplier,
          priority: 10,
        });
        return true;
      }
  ...
  protected checkTreasuryLow(
    ...
    if (usdcBalance >= minTreasuryBuffer) {
      thinking.push({
        category: "treasury",
        observation: `Treasury balance: ${this.formatUsdc(usdcBalance)}`,
        conclusion: `Above minimum buffer (${this.formatUsdc(minTreasuryBuffer)})`,
        metrics: {
    ...
    thinking.push({
      category: "treasury",
      observation: `Treasury LOW: ${this.formatUsdc(usdcBalance)} < minimum ${this.formatUsdc(minTreasuryBuffer)}`,
  ...
  protected checkRepayLoan(
    ...
    if (usdcBalance <= repayTrigger) {
      thinking.push({
        category: "lending",
        observation: `Treasury ${this.formatUsdc(usdcBalance)} below repay trigger ${this.formatUsdc(repayTrigger)}`,
    ...
    thinking.push({
      category: "lending",
      observation: `Treasury ${this.formatUsdc(usdcBalance)} > repay trigger ${this.formatUsdc(repayTrigger)}`,
      conclusion: `Repaying ${this.formatUsdc(repayAmount)} of ${this.formatUsdc(borrowedAmount)} loan`,
      metrics: {
    ...
    actions.push({
      ...
      params: {
        repayAmount,
        withdrawCollateral: repayAmount >= borrowedAmount,
      } as RepayParams,
      reason: `Excess treasury, repaying ${this.formatUsdc(repayAmount)}`,
      confidence: 0.8 * this.confidenceMultiplier,
      priority: 6,
    });
  ...
  protected checkClaimRevenue(
    ...
    if (ownTokenRevenue < MIN_CLAIM_THRESHOLD) {
      thinking.push({
        category: "revenue",
        observation: `Pending revenue: ${this.formatUsdc(ownTokenRevenue)}`,
        conclusion: `Below claim threshold (${this.formatUsdc(MIN_CLAIM_THRESHOLD)})`,
      });
      return false;
    }
    ...
    actions.push({
      ...
      reason: `Claiming ${this.formatUsdc(ownTokenRevenue)} in revenue`,
      confidence: 0.9 * this.confidenceMultiplier,
      priority: 5,
    });
  ...
  /**
   * Evaluate buy opportunities - can be overridden by subclasses
   * for different scoring algorithms
   */
  protected evaluateBuyOpportunities(
    state: AgentState,
    config: AgentConfig,
    thinking: ThinkingStep[],
    actions: Action[]
    ...
    if (usdcBalance <= targetTreasuryBuffer) {
      thinking.push({
        category: "trading",
        observation: `Treasury ${this.formatUsdc(usdcBalance)} <= target ${this.formatUsdc(targetTreasuryBuffer)}`,
    ...
    if (maxBuyAmount < ONE_USDC) {
      thinking.push({
        category: "trading",
        observation: `Speculation budget: ${this.formatUsdc(maxBuyAmount)}`,
        conclusion: "Budget too small for meaningful trades",
      });
      return false;
    }

    // Get sentiment multiplier (subclasses can override)
    const sentimentMultiplier = this.getSentimentMultiplier(marketSentiment);
    ...
    // Score opportunities (subclasses can override)
    const buyOpportunities = this.scoreBuyOpportunities(
      otherCreators,
      buySignals,
      sentimentMultiplier
    );
    ...
    actions.push({
      ...
      reason: `Buy opportunity: score ${bestOpportunity.score.toFixed(3)}, investing ${this.formatUsdc(buyAmount)}`,
      confidence: Math.min(0.7 + bestOpportunity.score, 0.9) * this.confidenceMultiplier,
      priority: 4,
    });
  ...
  /**
   * Evaluate sell opportunities - can be overridden by subclasses
   */
  protected evaluateSellOpportunities(
    state: AgentState,
    config: AgentConfig,
    thinking: ThinkingStep[],
    actions: Action[]
    ...
    for (const holding of holdings) {
      ...
      if (pnlPercent > sellSignals.profitTake * 100) {
        ...
        // Sell half to lock in profits (subclasses can override)
        const sellAmount = this.getProfitTakeSellAmount(holding);
        ...
        actions.push({
          ...
          reason: `Profit take: ${tokenName} at +${pnlPercent.toFixed(1)}%, selling ${sellAmount === balance ? "100%" : "50%"}`,
          confidence: 0.8 * this.confidenceMultiplier,
          priority: 3,
        });
        soldAny = true;
  ...
  /**
   * Get sentiment multiplier for buy decisions
   * Override in subclasses for different behavior
   */
  protected getSentimentMultiplier(
  ...
  /**
   * Score buy opportunities
   * Override in subclasses for different scoring algorithms
   */
  protected scoreBuyOpportunities(
    ...
    for (const creator of otherCreators) {
      ...
      const revenueScore =
        Number(creator.pendingRevenue) / Number(ONE_USDC) / 100;
  ...
  /**
   * Get sell amount for profit taking
   * Override in subclasses for different behavior
   */
  protected getProfitTakeSellAmount(holding: Holding): bigint {
    // Default: sell 50%
    return holding.balance / BigInt(2);
  }
}
